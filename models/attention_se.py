...
class HardNegativeMining(nn.Module):
    ...
    def forward(self, features):
        ...
        for i in range(batch_size):
            cosine_sim = F.cosine_similarity(feature_vectors, feature_vectors[i].unsqueeze(0), dim=1)
            cosine_sim[i] = -1  # 자기 자신 제외

            closest_indices = torch.topk(cosine_sim, self.num_negatives, largest=True).indices
            hard_negatives.append(features[closest_indices].mean(dim=0))
...
class EnhancedDistortionDetectionModel(nn.Module):
    def __init__(self, num_negatives=5):
        ...
        self.hard_neg_mining = HardNegativeMining(num_negatives)
    ...
    def forward(self, x):
        ...
        hard_negatives = self.hard_neg_mining(high_feat)
...
class HardNegativeMining(nn.Module):
    ...
    def forward(self, features):
        ...
        for i in range(batch_size):
            ...
            # ✅ 선택 가능한 최대 k 값으로 조정 (인덱스 초과 방지)
            k = min(self.num_negatives, len(cosine_sim) - 1)
            closest_indices = torch.topk(cosine_sim, k, largest=True).indices

            # ✅ Hard Negative 벡터 평균 사용
            if len(closest_indices) > 0:
                hard_negatives.append(features[closest_indices].mean(dim=0))
            else:
                hard_negatives.append(features[i])
