# Attribute Feature Processor와 Texture Block Processor 추가 전
...
# Test Metrics: {'srcc': 0.9382215517663918, 'plcc': 0.9435880483522886}
...
class DistortionAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Attention weights mean and std
        attention_mean = attention.mean().item()
        attention_std = attention.std().item()
        print(f"[Debug] DistortionAttention: mean={attention_mean:.6f}, std={attention_std:.6f}")
...
class HardNegativeCrossAttention(nn.Module):
    ...
    def forward(self, x_attr, x_texture):
        ...
        attention_mean = attention.mean().item()
        attention_std = attention.std().item()
        print(f"[Debug] HardNegativeCrossAttention: mean={attention_mean:.6f}, std={attention_std:.6f}")
...
# Attribute Feature Processor와 Texture Block Processor 추가 후
""" 
import sys
import os
...
def debug_tensor_shapes(x, name="Tensor"):
    print(f"[Debug] {name} shape: {x.shape}")

def debug_attention_weights(attention):
    print(f"[Debug] Attention weights mean: {attention.mean().item()}, std: {attention.std().item()}")
...
class DistortionAttention(nn.Module):
    ...
    def forward(self, x_attr, x_texture=None):
        if x_texture is None:
            x_texture = x_attr

        debug_tensor_shapes(x_attr, "x_attr")
        debug_tensor_shapes(x_texture, "x_texture")

        b, c, h, w = x_attr.size()
        query = self.query_conv(x_attr).view(b, -1, h * w).permute(0, 2, 1)
        key = self.key_conv(x_texture).view(b, -1, h * w)
        value = self.value_conv(x_texture).view(b, -1, h * w)

        attention = self.softmax(torch.bmm(query, key) * (c ** -0.5))
        debug_attention_weights(attention)
...
""" class HardNegativeCrossAttention(nn.Module):
    ...
    def forward(self, x_attr, x_texture):
        ...
        key = self.key_conv(x_texture).view(b, self.num_heads, head_dim, h * w).permute(0, 1, 3, 2)
        value = self.value_conv(x_texture).view(b, self.num_heads, head_dim, h * w).permute(0, 1, 3, 2)

        # Debug tensor shapes
        debug_tensor_shapes(query, "query")
        debug_tensor_shapes(key, "key")
        debug_tensor_shapes(value, "value")
...
class HardNegativeCrossAttention(nn.Module):
    ...
    def forward(self, x_attr, x_texture):
        ...
        # Reshape query, key, and value
        query = self.query_conv(x_attr).view(b, self.num_heads, head_dim, h * w).permute(0, 1, 3, 2)
        key = self.key_conv(x_texture).view(b, self.num_heads, head_dim, h * w).permute(0, 1, 2, 3)
        value = self.value_conv(x_texture).view(b, self.num_heads, head_dim, h * w).permute(0, 1, 3, 2)

        # Ensure query and key have matching dimensions
        assert query.size(-2) == key.size(-1), (
            f"Query and Key dimensions mismatch: query size {query.size()}, key size {key.size()}"
        )

        # Scale factor for stability
        scale = head_dim ** -0.5
        attention = self.softmax(torch.matmul(query, key) * scale)  # Shape: (b, num_heads, h*w, h*w)
        out = torch.matmul(attention, value).permute(0, 1, 3, 2).contiguous()
