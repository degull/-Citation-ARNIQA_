...
class SimCLR(nn.Module):
    ...
    def forward(self, inputs_A, inputs_B=None, return_maps=False):
        # Reshape inputs if 5D
        if inputs_A.dim() == 5:
            inputs_A = inputs_A.view(-1, *inputs_A.shape[2:])
        if inputs_B is not None and inputs_B.dim() == 5:
        ...
        features_A = self.backbone(inputs_A)
        se_weights_A = features_A.mean(dim=[2, 3])  # SE weights for inputs_A
        spatial_map_A = self.distortion_attention._generate_spatial_map(features_A)
        channel_map_A = self.distortion_attention._generate_channel_map(features_A)

        features_A = self.distortion_attention(
            features_A, se_weights=se_weights_A, spatial_map=spatial_map_A, channel_map=channel_map_A
        )
        ...
        if inputs_B is not None:
            ...
            features_B = self.backbone(inputs_B)
            se_weights_B = features_B.mean(dim=[2, 3])  # SE weights for inputs_B
            spatial_map_B = self.distortion_attention._generate_spatial_map(features_B)
            channel_map_B = self.distortion_attention._generate_channel_map(features_B)

            features_B = self.distortion_attention(
                features_B, se_weights=se_weights_B, spatial_map=spatial_map_B, channel_map=channel_map_B
            )
            ...
            if return_maps:
                return proj_A, proj_B, spatial_map_A, channel_map_A, spatial_map_B, channel_map_B
            return proj_A, proj_B
        ...
        if return_maps:
            return proj_A, None, spatial_map_A, channel_map_A, None, None
        return proj_A, None
