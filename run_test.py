...
if __name__ == "__main__":
    ...
    # ✅ 저장된 모델 불러오기
    model_path = "E:/ARNIQA - SE - mix/ARNIQA/experiments/my_experiment/regressors/epoch_27_srocc_0.920.pth"
    model = KADID10KDataset().to(device)
    model.load_state_dict(torch.load(model_path, map_location=device))
...
from data.dataset_koniq10k import KONIQ10KDataset
from data.dataset_live import LIVEDataset
from models.attention_se import MultiTaskIQA  # ✅ 모델 수정
from utils.utils import load_config
...
def test(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            preds, _ = model(img_A)  # ✅ MOS 예측만 사용
            srocc, plcc = calculate_srcc_plcc(preds, targets)
...
if __name__ == "__main__":
    ...
    # ✅ 저장된 모델 불러오기
    model_path = "E:/ARNIQA - SE - mix/ARNIQA/experiments/my_experiment/regressors/epoch_27_srocc_0.920.pth"
    model = MultiTaskIQA().to(device)  # ✅ 모델 인스턴스 생성
    model.load_state_dict(torch.load(model_path, map_location=device))

    # ✅ 데이터셋 로드 (경로 전달 추가)
    datasets = {
        "KADID10K": KADID10KDataset(root=args.data_base_path_kadid, crop_size=224),
        "TID2013": TID2013Dataset(root=args.data_base_path_tid, crop_size=224),
        "SPAQ": SPAQDataset(root=args.data_base_path_spaq, crop_size=224),
        "CSIQ": CSIQDataset(root=args.data_base_path_csiq, crop_size=224),
        "CLIVE": CLIVEDataset(root=args.data_base_path_clive, crop_size=224),
        "KONIQ10K": KONIQ10KDataset(root=args.data_base_path_koniq, crop_size=224),
        "LIVE": LIVEDataset(root=args.data_base_path_live, crop_size=224),
    }
