...
def train(args, model, train_dataloader, val_dataloader, test_dataloader, optimizer, lr_scheduler, device):
    ...
    train_metrics = {'loss': []}
    val_metrics = {'srcc': [], 'plcc': []}
    test_metrics = {'srcc': [], 'plcc': []}  # 모든 에포크에 대한 테스트 결과 저장
    ...
    for epoch in range(args.training.epochs):
        ...
        for batch in progress_bar:
            ...
            with torch.cuda.amp.autocast():
                ...
                features_negatives = model.backbone(hard_negatives).mean([2, 3])
                proj_negatives = F.normalize(model.projector(features_negatives), dim=1)
...
def test(args, model, test_dataloader, device):
    ...
    return {'srcc': avg_srocc, 'plcc': avg_plcc}
...
if __name__ == "__main__":
    ...
    # KADID10K 데이터셋 로드
    kadid_dataset_path = Path(args.data_base_path_kadid)
    ...
    # TID2013 데이터셋 로드
    tid_dataset_path = Path(args.data_base_path_tid)
